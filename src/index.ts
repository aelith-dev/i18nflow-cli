...
program
  ...
  .action(async () => {
    ...
    const targetDirAbs = path.resolve(process.cwd(), localesDir as string);
...
program
  ...
  .action(async (locale) => {
    ...
    const localesDirAbs = path.resolve(process.cwd(), config.localesDir);
    const targetFilePath = path.join(localesDirAbs, `${locale}.json`);
    const updatedTargets = config.targets.filter((t: string) => t !== locale);
...
function initialBaseContent(locale: string) {
  switch (locale) {
    case 'en':
      return { hello: 'Hello' };
    case 'es':
